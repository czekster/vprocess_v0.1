 ...
 * Description: class Simulator header
 *
 */
...
class Simulator {
   public:
      Simulator();
      ~Simulator();
      void run(ulong totalEvts, double* rates, int iter);   // runs the simulation for totalEvts events
      void showEvents();
   protected:
   private:
      double globalClock;                                   // global clock for the simulation
      list<Event*> events;                                  // list of events
      RandomGenerator* randGen;                             // its own random generator
      CPU* cpu;
      Statistics* statistics;
      discreteevent draw(double* rates, double* when);      // randomly selects (draws) the next event to fire
      void addEvent(Event* e);
      string eventStr(discreteevent e);
};
